// ==UserScript==
// @id             iitc-plugin-deploychallenge
// @name           IITC plugin: Umbra Deploy Challenge
// @category       Misc
// @version        0.0.1.20191220.160800
...
// @updateURL      https://raw.githubusercontent.com/AG739107/iitc-plugins/master/devploychallenge.user.js
// @downloadURL    https://raw.githubusercontent.com/AG739107/iitc-plugins/master/devploychallenge.user.js
...
function wrapper(plugin_info) {
  ...
  window.plugin.aurora.updateCheckedAndHighlight = function (guid) {
    if (guid === window.selectedPortal) {
      ...
      $('#glyph_6').prop('checked', glyphed & glyph[6]);
      $('#glyph_7').prop('checked', glyphed & glyph[7]);
      $('#glyph_8').prop('checked', glyphed & glyph[8]);
    }
  ...
  window.plugin.aurora.highlighter = {
    highlight: function (data) {
      ...
      if (glyphInfo) {
        ...
        if (glyphInfo.glyphed & glyph[6]) total += 1;
        if (glyphInfo.glyphed & glyph[7]) total += 1;
        if (glyphInfo.glyphed & glyph[8]) total += 1;
  ...
  window.plugin.aurora.countTotal = function () {
    ...
    for (var guid in data) {
      ...
      if (glyphed & glyph[6]) total += 1;
      if (glyphed & glyph[7]) total += 1;
      if (glyphed & glyph[8]) total += 1;
    }
  ...
  window.plugin.aurora.showList = function () {
    ...
    html += '<tr><th>#</th><th>Portal Name</th>'
      ...
      + '<th><label><input type="checkbox" class="glyph_all" value="5"> 5</label></th>'
      + '<th><label><input type="checkbox" class="glyph_all" value="6"> 6</label></th>'
      + '<th><label><input type="checkbox" class="glyph_all" value="7"> 7</label></th>'
      + '<th><label><input type="checkbox" class="glyph_all" value="8"> 8</label></th></tr>';
    ...
    for (var i = 0; i < portals.length; i++) {
      ...
      html += '<tr><td>' + (i + 1) + '</td>'
        ...
        + '<td><label><input type="checkbox" class="glyph_5" data-guid="' + guid + '" onclick="window.plugin.aurora.updateGlyphed($(this).prop(\'checked\'), 5, \'' + guid + '\')"' + (data & glyph[5] ? ' checked' : '') + '> 5</label></td>'
        + '<td><label><input type="checkbox" class="glyph_6" data-guid="' + guid + '" onclick="window.plugin.aurora.updateGlyphed($(this).prop(\'checked\'), 6, \'' + guid + '\')"' + (data & glyph[6] ? ' checked' : '') + '> 6</label></td>'
        + '<td><label><input type="checkbox" class="glyph_7" data-guid="' + guid + '" onclick="window.plugin.aurora.updateGlyphed($(this).prop(\'checked\'), 7, \'' + guid + '\')"' + (data & glyph[7] ? ' checked' : '') + '> 7</label></td>'
        + '<td><label><input type="checkbox" class="glyph_8" data-guid="' + guid + '" onclick="window.plugin.aurora.updateGlyphed($(this).prop(\'checked\'), 8, \'' + guid + '\')"' + (data & glyph[8] ? ' checked' : '') + '> 8</label></td></tr>';
    }
